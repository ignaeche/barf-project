...
def read_handler(event, process, ir_emulator, initial_taints, open_files, file_mem_mapper, addrs_to_file):
    # Get parameters.
...
def process_event(process, event, ir_emulator, initial_taints, open_files, file_mem_mapper, addrs_to_file):
    if isinstance(event, CallIntel32):
        ...
        if event.name == "read":
            read_handler(event, process, ir_emulator, initial_taints, open_files, file_mem_mapper, addrs_to_file)
